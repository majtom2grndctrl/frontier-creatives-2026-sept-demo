...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The keyboard and ARIA sections are still accurate. The prop tables (`ariaLabel`, `componentRef`) and the `react-texture` composition model are stale and do not exist in v9.
