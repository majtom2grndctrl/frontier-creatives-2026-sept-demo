...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. Text's role as a shared text-styling primitive still holds. The prop table is stale — it documents a single `variant` prop, while the shipped v9 `Text` uses `size`, `weight`, `align`, `font`, `truncate`, and `wrap`.
