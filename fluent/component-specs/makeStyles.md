...
> **Historical rationale.** Explains why Fluent moved to atomic CSS-in-JS, from Fela through v8 `makeStyles` to Griffel. Background reading rather than a usage guide — for writing styles today, use `makeStyles` from `@fluentui/react-components`.
