...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The core idea of reserving layout space for asynchronous content still holds. The API is stale — v9 ships `Skeleton` and `SkeletonItem`; there is no `Skeleton.Line`, no `Skeleton.Shape`, and no `animation` prop.
