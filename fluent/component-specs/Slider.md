...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The `role="slider"` and arrow/Home/End keyboard handling are still accurate. The prop tables and the `.compose()` model are stale and do not reflect the shipped v9 API.
