...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The WAI-ARIA grid pattern and the Fabric-to-Fluent addendum are still accurate. The prop table and internal structure (`InputBlock`, `CalendarControls`) are stale and don't match the shipped `@fluentui/react-datepicker-compat`.
