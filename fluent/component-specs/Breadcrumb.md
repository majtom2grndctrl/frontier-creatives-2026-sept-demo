...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The accessibility section (`aria-current`, nav landmark) is still accurate. The dot-notation subcomponents are stale — v9 exports `BreadcrumbItem`, `BreadcrumbButton`, and `BreadcrumbDivider` as separate named imports, not `Breadcrumb.Item`.
