...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The ARIA, indeterminate state, and keyboard sections are still accurate. The prop tables and the `react-texture` composition example are stale and do not reflect the shipped v9 API.
