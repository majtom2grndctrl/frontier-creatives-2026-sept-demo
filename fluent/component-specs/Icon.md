...
> **Describes the v8 pattern, not v9.** Fluent v9 has no name-registered `Icon` component and no `initializeIcons()`. Icons are individually imported tree-shakeable components from `@fluentui/react-icons`, for example `import { Home24Regular } from '@fluentui/react-icons'`. Read this only for the accessibility guidance.
