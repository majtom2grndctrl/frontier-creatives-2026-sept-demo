...
> **Historical RFC.** A pre-implementation design proposal from the Fabric to v9 convergence, not current API documentation. The behaviors, ARIA, and keyboard interaction sections are still accurate. The cross-library DOM comparison tables, the Fabric prop-conversion tables, and the `react-texture` / `BaseButton.compose()` composition model are stale — that composition model does not exist in v9.
